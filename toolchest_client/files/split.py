...
def open_new_output_file(
    ...
    current_output_file_path = (
        f"{working_directory}/{filename_prefix}_{current_split_number}_{input_basename}"
    )
    return current_output_file_path, open(current_output_file_path, "w", newline="\n")


def split_file_by_lines(
    input_file_path, num_lines_in_group=4, max_bytes=4.5 * 1024 * 1024 * 1024
):
    ...
    for line in large_input_file:
        if need_to_open_new_file:
            current_output_file_path, current_output_file = open_new_output_file(
                current_split_number=current_split_number, input_basename=input_basename
            )
        ...
        if (
            current_output_bytes >= max_bytes
            and current_line_number % num_lines_in_group == 0
        ):
...
def split_paired_files_by_lines(
    input_file_paths, num_lines_in_group=4, max_bytes=4.5 * 1024 * 1024 * 1024
):
    ...
    for _, [read_one_file_path, read_two_file_path] in grouped_input_file_paths.items():
        ...
        for grouped_files_paths_with_counts in zip(
            read_one_file_paths, read_two_file_paths
        ):
            (read_one_lines, split_read_one_file_path), (
                read_two_lines,
                split_read_two_file_path,
